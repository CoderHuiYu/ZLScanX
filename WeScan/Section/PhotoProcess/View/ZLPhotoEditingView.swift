...
protocol ZLPhotoEditingViewDelegate: NSObjectProtocol {
...
class ZLPhotoEditingView: UIView {
    ...
    weak var delegate: ZLPhotoEditingViewDelegate?
    ...
    @IBAction func toolBarItemAction(_ sender: UIButton) {
        if let delegate = delegate {
            delegate.toolBarItemAction(index: sender.tag)
        }
    }
