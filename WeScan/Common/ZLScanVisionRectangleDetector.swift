//
//  ZLScanVisionRectangleDetector.swift
//  WeScan
//
...
@available(iOS 11.0, *)
struct ZLScanVisionRectangleDetector {
    ...
    static func rectangle(forImage image: CIImage, completion: @escaping ((ZLQuadrilateral?) -> Void)) {
        ...
        let rectangleDetectionRequest: VNDetectRectanglesRequest = {
            let rectDetectRequest = VNDetectRectanglesRequest(completionHandler: { (request, error) in
                ...
                let quads: [ZLQuadrilateral] = results.map({ observation in
                    return ZLQuadrilateral(topLeft: observation.topLeft, topRight: observation.topRight, bottomRight: observation.bottomRight, bottomLeft: observation.bottomLeft)
                })
